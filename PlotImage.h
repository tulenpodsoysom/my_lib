...
#undef max
	...
	public:
		struct accommodate_settings
		{
			...
			enum plot_bounds_detect
			{
				...
				linear,
				max_min
			} bounds_detection_x, bounds_detection_y;
		} acc_settings;

		void accommodate(Gdiplus::Rect outerrect);
		void accommodate(accommodate_settings settings, Gdiplus::Rect outerrect);
		...
		std::wstring title;
		std::wstring captionX;
		std::wstring captionY;
	...
	template<typename InputIt>
	inline void PlotImage::add_points(InputIt begin, InputIt end, float width, float left)
	{
	...
	template<typename InputIt>
	inline void PlotImage::add_points(std::initializer_list<std::pair<InputIt, InputIt>> list, float width, float left)
	{
		for (auto& i : list)
		{
	...
	template<typename InputIt>
	inline void PlotImage::set_points(InputIt begin, InputIt end, float width, float left)
	{
	...
	template<typename InputIt>
	inline void PlotImage::set_points(std::initializer_list<std::pair<InputIt, InputIt>> list, float width, float left)
	{
